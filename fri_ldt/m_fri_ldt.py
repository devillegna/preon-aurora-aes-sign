...
def ldt_commit_phase( f0 , h_state , RS_rho=8 , verbose = 1 ):
    ...
    while( 2 < poly_len ):
        ...
        dump( "ibtfy_1:" , hex(offset) , f"|{len(vi)}|" )
        vi_e = vi[::2]
        vi_o = vi[1::2]
        vi = [ vi_e[j]^gf.mul(vi_o[j],xi) for j in range(len(vi_e)) ]
        dump( f"|n_vi|: {len(vi)}" )   # , [hex(e) for e in vi] )
...
def ldt_verify_proof( commits , d1poly , open_mesgs , xi , queries , Nq , verbose = 1 ):
    ...
    for auths in open_mesgs :
        ...
        if 0!=j :
            verify_j = [ _check_linear_relation(last_mesg[i],mesg[i],queries[i],xi[j],offset) for i in range(Nq) ]
        ...
        j = j+1
    # check deg 1 poly
    verify_j = [ _check_deg1poly_linear_relation(last_mesg[i],d1poly,queries[i],xi[-1],offset) for i in range(Nq) ]
    dump( f"check last linear relations (with the d1poly):" , all(verify_j) )
    if not all(verify_j) : return False
    return True

def _check_linear_relation( mesgjm1 , mesgj , idx , xi , offset ) :
